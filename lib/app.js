...
// app.getGamesOfPlayer = function(gameName, playerUsername, callback) {
//   app.getGame(gameName, function(err, game) {
//     if(err) return callback(err);
//     app.getPlayer(playerUsername, function(err, player) {
//       if(err) return callback(err);
//       Score.findOne({ game: game._id, player: player._id }, function(err, score) {
//         if(err) return callback(err);
//         else if(!score) return callback(err);
//         return callback(null, score);
//       });
//     });
//   });
...
app.getGamesOfPlayer = function(playerUsername, callback) {
  app.getPlayer(playerUsername, function(err, player) {
  if(err) return callback(err);
    Score.findOne({ player: player._id })
      .populate('game')
      .exec(function(err, scores) {
        if(err) return callback(err);
        // console.log(Object.keys(scores));
        var games = Object.keys(scores).forEach(function(score) {
          if(!scores.hasOwnProperty(score)) return score.game;
        });
        // console.log(Object.keys(scores));
        // var games = [];
        // for(var score in scores) {
        //   if(!scores.hasOwnProperty(score)) continue;
        // //   var obj = scores[score];
        // //   for(var prop in scores) {
        // //     if(!obj.hasOwnProperty(prop)) continue;
        //     games.push(scores.score.game);
        //     console.log(scores.score.game);
        // //   }
        // //   // else {
        // //   //   console.log(scores);
        // //   //   // console.log(games);
