...
app.getUser = function(userId, callback) { //returns nothing (error is empty-null) when getting smth non-existent
  User.findById(userId, function(err, user) {
    if(err) return callback(err);
    if(!user) return callback('User not found.'); //APPLY THIS TO OTHERS
    return callback(null, user); //RETURN AND KEEP IT CONSISTENT
  });
};
...
app.deletePlayer = function(playerId, callback) {
  Player.remove({ _id: playerId }, function(err, player) { //player burda (delete func) olur mu olmaz mi
    if(err) return callback(err);
