...
app.deleteUser = function(userId, callback) {
...
app.updateGame = function(gameId, name, website, callback) {
  Game.findById(gameId, function(err, game) {
    if(err) return callback(err);
    if(!game) return callback('Game not found.');
    if(game.name === name && game.website === website) return callback('Not updated info.');
    if(game.name != name) game.name = name;
...
app.getTopTenScores = function(game, callback) {
  Score.find({ game: game })
    .sort({ 'score': -1 })
    .limit(10)
    .populate('player')
...
app.getScore = function(game, player, callback) {
  Score.findOne({ game: game, player: player }, function(err, score) {
    if(err) return callback(err);
    return callback(null, score);
  });
};

app.createScore = function(game, player, newScore, callback) {
  var score = new Score();
  score.game = game;
  score.player = player;
...
app.updateScore = function(game, player, newScore, callback) {
  Score.findOne({ game: game, player: player }, function(err, score) {
    if(err) return callback(err);
    if(!score) return callback('No previous score.');
    if(score.score === newScore) return callback('Not updated info.');
    score.score = newScore;
